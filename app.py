...
if input_mode == "Manual Entry":
    # 1. Dynamic Selectors (Outside form to ensure responsiveness)
    st.subheader("Step 1: Crop Selection")
    ...
    # 2. Form for other parameters
    st.subheader("Step 2: Environmental & Soil Parameters")
    ...
    if submit_button:
        try:
            ...
            st.write("#### Data used for prediction:", pred_df)
        except Exception as e:
            st.error(f"Error during prediction: {e}")

else:
    # CSV Upload Mode
    st.subheader("Bulk Batch Prediction")
    uploaded_file = st.file_uploader("Upload your Greenhouse CSV file", type=["csv"])
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("Run Bulk Prediction"):
                ...
                # Predict (Note: Ensure CSV column names match exactly what CustomData produces)
                results = predict_pipeline.predict(df)
                
                # Add results to dataframe
                
                df['predicted_yield_kg_per_m2'] = results
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name="greenhouse_predictions.csv",
                    mime="text/csv"
                )
